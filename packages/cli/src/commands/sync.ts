  ...
  .action(async (slug: string | undefined, options) => {
    try {
      ...
      if (!config.sync.target) {
        ...
        console.error('Run "pkm sync config" to set up Docusaurus sync.');
        process.exit(1);
      }
      ...
      const docusaurusSync = new DocusaurusSync(config, pkmRoot);
    ...
    try {
      ...
      const answers = await inquirer.prompt([
        ...
        },
        {
          ...
          default: config.sync.assetsPath || join(config.sync.target, '..', 'static', 'img'),
        },
        {
          type: 'confirm',
          name: 'enabled',
      ...
      await configManager.setConfigValue('sync.transformFrontmatter', answers.transformFrontmatter);
      await configManager.setConfigValue('sync.copyAssets', answers.copyAssets);

      console.log('\n✓ Sync configuration updated!');
      console.log('\nNext steps:');
      console.log('  1. Mark notes for publishing: Add "publish: true" to frontmatter');
      console.log('  2. Preview sync: pkm sync --dry-run');
      console.log('  3. Sync notes: pkm sync');
...
syncCommand
  ...
  .action(async () => {
    try {
      ...
      console.log(`Assets Path: ${config.sync.assetsPath || '(not configured)'}`);

      // Find publishable notes
      const allNotes = await noteManager.listNotes();
      const publishableNotes = allNotes.filter(note => note.frontmatter.publish === true);
      ...
      if (publishableNotes.length > 0) {
        for (const note of publishableNotes) {
          ...
          console.log(`    ${note.frontmatter.category} • ${note.frontmatter.tags.join(', ')}`);
          console.log();
        }
      } else {
