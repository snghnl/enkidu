...
import { readFile, writeFile, deleteFile } from "../utils/fs.js";
  ...
  .addCommand(
    new Command("create")
      ...
      .action(
        async (
          name: string,
          options: { description?: string; basedOn?: string },
        ) => {
          try {
            const config = await ConfigManager.load();
            const loader = new TemplateLoader(config.rootDir);
            ...
            try {
              ...
              console.error(
                chalk.red(
                  `Template "${name}" already exists. Use 'edit' to modify it.`,
                ),
              );
              process.exit(1);
            } catch {
            ...
            if (options.basedOn) {
              try {
                ...
                console.error(
                  chalk.red(`Base template "${options.basedOn}" not found.`),
                );
                process.exit(1);
              }
            } else {
...
`;
            ...
            const tempFilePath = `${config.rootDir}/.temp-${name}.md`;

            try {
              // Write to temp file for editor
              writeFile(tempFilePath, baseContent);

              // Open in editor
              await openInEditor(tempFilePath, config.editor);

              // Read edited content
              const editedContent = readFile(tempFilePath);

              if (!editedContent || editedContent.trim() === "") {
                console.error(
                  chalk.red(
                    "Template creation cancelled - no content provided.",
                  ),
                );
                process.exit(1);
              }
      ...
      .action(async (source: string, destination: string) => {
        try {
          ...
          try {
            ...
            console.error(
              chalk.red(`Template "${destination}" already exists.`),
            );
            process.exit(1);
          } catch {
          ...
          console.log(
            chalk.cyan(
              `   You can now edit it with: enkidu template edit ${destination}\n`,
            ),
          );
        } catch (error) {
    ...
    new Command("edit")
      ...
      .action(async (name: string) => {
        try {
          ...
          // Open in editor using temp file
          console.log(chalk.cyan("\n📝 Opening template in editor...\n"));
          const tempFilePath = `${config.rootDir}/.temp-edit-${name}.md`;

          try {
            // Write template content to temp file
            writeFile(tempFilePath, template.content);
    ...
    new Command("delete")
      ...
      .action(async (name: string, options: { force?: boolean }) => {
        try {
          ...
          if (template.isBuiltIn) {
            console.error(
              chalk.red(`Cannot delete built-in template "${name}".`),
            );
            process.exit(1);
          }
