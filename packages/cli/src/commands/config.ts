...
export const configCommand = new Command('config')
  .description('Manage PKM configuration')
  .addCommand(
    new Command('get')
      ...
      .action(async (key: string) => {
        try {
          ...
          console.error('Error getting config:', error instanceof Error ? error.message : error);
          process.exit(1);
        }
      })
  )
    ...
    new Command('list')
      ...
      .action(async () => {
        try {
          ...
          console.error('Error listing config:', error instanceof Error ? error.message : error);
          process.exit(1);
        }
      })
  )
    ...
    new Command('edit')
      ...
      .action(async () => {
        try {
          ...
          const configPath = `${config.rootDir}/.pkm/config.json`;

          // Open in editor
          await execa(editor, [configPath], { stdio: 'inherit' });
        } catch (error) {
          console.error('Error opening config editor:', error instanceof Error ? error.message : error);
          process.exit(1);
        }
      })
  );
