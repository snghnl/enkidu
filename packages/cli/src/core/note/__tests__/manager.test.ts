...
describe("NoteManager", () => {
  ...
  describe("updateNote", () => {
    ...
    it("should update the updated timestamp", async () => {
      ...
      expect(updated.frontmatter.updated).not.toBe(
        original.frontmatter.updated,
      );
    });
  ...
  describe("listNotes", () => {
    ...
    it("should filter by category", async () => {
      ...
      expect(notes.every((n) => n.frontmatter.category === "general")).toBe(
        true,
      );
    });
    ...
    it("should filter by tag", async () => {
      ...
      expect(notes.length).toBe(2);
      expect(notes.every((n) => n.frontmatter.tags.includes("tag1"))).toBe(
        true,
      );
    });
