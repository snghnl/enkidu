...
describe('ConfigManager', () => {
  ...
  describe('loadConfig', () => {
    ...
    it('should merge custom config with defaults', async () => {
      ...
      expect(config.pkm.notesDir).toBe('custom-notes');
    ...
    it('should validate config schema', async () => {
      ...
      expect(config.pkm.notesDir).toBe('notes');
      expect(config.pkm.templatesDir).toBe('templates');
    });
  ...
  describe('saveConfig', () => {
    ...
    it('should preserve all config values', async () => {
      ...
      expect(loadedConfig.pkm).toEqual(originalConfig.pkm);
    });
  });
  ...
  describe('findPkmRoot', () => {
    it('should find root in current directory', async () => {
      await configManager.initConfig(tempDir);

      const root = ConfigManager.findPkmRoot(tempDir);
    ...
    it('should find root in parent directory', async () => {
      ...
      const root = ConfigManager.findPkmRoot(subDir);
    ...
    it('should find root in deep nested directory', async () => {
      ...
      const root = ConfigManager.findPkmRoot(deepDir);
    ...
    it('should use process.cwd() by default', async () => {
      ...
      const root = ConfigManager.findPkmRoot();

      expect(root === null || typeof root === 'string').toBe(true);
    });

    it('should stop at filesystem root', () => {
      const root = ConfigManager.findPkmRoot('/tmp/non-existent-path-xyz');
